...
- DeepSeek Harness now targets the official `deepseek-harness-sdk==0.1.2a3` and matching `deepseek-harness-runtime-bin==0.1.2a3` API (#1521). The bridge supplies an explicit SDK home and `profile: sdk`, forwards `reasoning_effort`, and translates the trusted Cordis path to the new SDK `patches` option. Older SDKs without these constructor fields fail fast with an upgrade message.
