...
> **注意**: SDK provider（Claude SDK、Codex、OpenCode、Pi）の認証情報を設定すれば、対応する CLI のインストールは不要です。TAKT が API を直接呼び出します。`deepseek-harness` はさらに Python 3.10+、`deepseek-harness-sdk==0.1.2a3` と `deepseek-harness-runtime-bin==0.1.2a3`、Linux x64/arm64 または macOS arm64 が必要です。Windows と macOS x64 は未対応です。Cursor、Copilot、Kiro は CLI のインストールが必要です。
