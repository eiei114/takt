...
- 当显式配置的 extension 注册与 builtin 同名的 tool 时，该 extension tool 会在以下 4 个分支中替换 builtin：`readonly`、`edit`、未指定 permission mode 且显式指定 `allowedTools`，以及 `full` 且 `allowedTools` 仅包含 readonly tool。在 `readonly` 和 `edit` 中，mode 边界决定适用的名称，`allowedTools: []` 仍会拒绝所有 tool；在其他两个分支中，tool 名称必须包含在 `allowedTools` 中。ambient extension 与无法验证的 provenance 仍然会被拒绝。这种替换信任来自 SDK 的 allowlist，而不是操作系统 sandbox。
