...
- 明示的に設定した extension が builtin と同名の tool を登録した場合、その extension tool は `readonly`、`edit`、permission mode 未指定で明示的な `allowedTools` を指定した場合、および `full` で readonly tool だけを含む `allowedTools` を指定した場合に builtin を置き換えます。`readonly` と `edit` では mode の境界が対象を決め、`allowedTools: []` はすべての tool を拒否します。他の 2 分岐では、tool 名を `allowedTools` に含める必要があります。ambient の extension と検証できない provenance は引き続き拒否します。他の Pi permission 判定と同様に、この上書きの信頼も SDK の allowlist であり、OS sandbox ではありません。
