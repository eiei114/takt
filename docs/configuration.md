...
- When an explicitly configured extension registers a tool whose name matches a builtin, that extension tool replaces the builtin in `readonly`, `edit`, an unset permission mode with an explicit `allowedTools` list, and `full` with a readonly-only `allowedTools` list, as in plain Pi. In `readonly` and `edit`, `allowedTools: []` still denies every tool; in the other two branches, the tool name must be included in `allowedTools`. Ambient extensions and unverifiable provenance remain rejected. As with every Pi permission decision, this override trust is an SDK allowlist, not an operating-system sandbox.
