...
Permission controls and tool restrictions are not ignored: requests with `permissionMode` set, `bypassPermissions: true`, or an explicit `allowedTools` value (workflow `allowed_tools`, including an empty list) return `status: 'error'` before the bridge starts. Use a compatible provider when these constraints are required.
